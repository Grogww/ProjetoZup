...
const create = async (req, res, next) => {
  try {
    const { category_id, name, slug, description, icon, is_active } = req.body || {};
    ...
    if (err.code === 'SUBCATEGORY_SLUG_CONFLICT') {
      return res.status(409).json({ error: err.message });
    }
    next(err);
  }
...
const update = async (req, res, next) => {
  try {
    ...
    const subcategory = await subcategoriesService.updateSubcategory(id, patch);
    if (!subcategory) {
      return res.status(404).json({ error: 'Subcategory not found' });
    }
